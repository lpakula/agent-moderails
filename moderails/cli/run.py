...
def run_list(task_id, show_all, project_id, limit):
    ...
    Examples:
      moderails run list --task abc123
      moderails run list
...
def run_show(run_id):
    ...
    Example: moderails run show abc12345
    """
    session = _get_session()
    try:
        run_svc = RunService(session)
...
def run_logs(run_id, follow, raw):
    ...
    Example: moderails run logs abc12345
             moderails run logs abc12345 --follow
    """
...
def run_complete(summary):
    ...
    Example: moderails run complete --summary "$(cat <<'EOF'
    ## What was done
    ...
    EOF
    )"
    ...
    if not task_id:
        click.echo("No task_id found in .moderails/task_id", err=True)
        raise SystemExit(1)
