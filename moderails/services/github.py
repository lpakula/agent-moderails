"""GitHub integration -- polls issue comments for @moderails triggers."""
...
logger = logging.getLogger("moderails.github")
...
class GitHubService:
    ...
    def poll_integration(self, integration: Integration, session) -> int:
        """Poll a GitHub integration for @moderails comments. Returns count of tasks created/updated."""
        ...
        for comment in comments:
            ...
            # use the text from the @moderails comment (excluding the command itself)
            is_first_run = not existing_task
            if is_first_run:
                user_prompt = task.description
            else:
                raw_body = comment.get("body", "")
                user_prompt = re.sub(r"@moderails\b[^\n]*", "", raw_body).strip()
            ...
            if not user_prompt:
                ...
                self.comment_on_issue(
                    ...
                    "or include instructions after `@moderails`, e.g. `@moderails fix the login form`.",
                )
                continue
    ...
    def post_run_result(self, repo: str, task: Task, run_id: str,
        ...
        if pr:
            ...
            self.comment_on_issue(
                repo, issue_number,
                f"moderails completed run `{run_id}`. PR: #{pr_number}",
            )
            # Add backlink on PR (only if no prior moderails comment exists)
            self.comment_on_pr(
                repo, pr_number,
                f"Created by moderails from #{issue_number}",
            )
        else:
            short_summary = (summary[:200] + "...") if len(summary) > 200 else summary
            self.comment_on_issue(
                repo, issue_number,
                f"moderails completed run `{run_id}`.\n\n{short_summary}" if short_summary
                else f"moderails completed run `{run_id}`.",
            )
