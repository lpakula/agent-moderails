---
name: moderails-flows
description: Create and edit moderails flow definitions. Use when the user wants to create a new flow, modify flow steps, or asks about flow JSON format, step content structure, or flow best practices.
---
...
Flow files live in `moderails/flows/` (library) or `moderails/defaults/flows.json` (seeded defaults).
...
Gates are optional shell commands on a step that `moderails mode next` enforces before advancing. If any gate command exits non-zero, the agent is blocked and shown the failure.
...
**Artifacts** — if steps need to share state, use a file (e.g. `.moderails/task.md`). Early steps write findings; later steps read them.
...
2. **Create the JSON file** in `moderails/flows/<name>.json`
...
1. **Read the current flow** — check `moderails/defaults/flows.json` for seeded flows, or `moderails/flows/` for library flows
