...
if [ "$1" = "--rebuild" ]; then
    ...
    docker build -f "$PROJECT_ROOT/Dockerfile.test" -t moderails-test "$PROJECT_ROOT"
elif ! docker image inspect moderails-test >/dev/null 2>&1; then
...
docker run --rm \
  ...
  -w /app \
  moderails-test \
  pytest --cov=moderails --cov-report=term-missing -v "$@"
